...
Quaternion::Quaternion(const Vector4 &point)
    { // copy point
...
Quaternion::Quaternion(const Matrix4 &matrix)
    ...
    float w = sqrt(trace * 0.25f);
    // now we can compute the vector component from symmetric
    // pairs of entries
    // (2yz + 2xw) - (2yz - 2xw) = 4 xw 
    float x = 0.25f * (matrix.coordinates[1][2] - matrix.coordinates[2][1]) / w;
    // (2xz + 2yw) - (2xz - 2yw) = 4 yw 
    float y = 0.25f * (matrix.coordinates[2][0] - matrix.coordinates[0][2]) / w;
    // (2xy + 2zw) - (2xy - 2zw) = 4 zw 
    float z = 0.25f * (matrix.coordinates[0][1] - matrix.coordinates[1][0]) / w;
...
// Acts on a vector
Vector3 Quaternion::Act(const Vector3 &vector) const
    { // Act()
    // compute the result
    Quaternion resultQuat = Inverse() * Quaternion(vector) * (*this);
    Vector3 resultVector(resultQuat.coords[0], resultQuat.coords[1],
                         resultQuat.coords[2]);
...
Vector4 Quaternion::Act(const Vector4 &point) const
    { // Act()
    Quaternion resultQuat = Inverse() * Quaternion(point) * (*this);
    Vector4 resultPoint(resultQuat.coords[0], resultQuat.coords[1],
...
float Quaternion::AngleOfAction() const
    ...
    return (2.0f * acos(coords[3] / sqrtNorm));
    } // AngleOfAction()
...
Vector3 Quaternion::AxisOfRotation() const
    ...
    float theta = thetaDeg * 2.0f * float(3.1415926) / 360.0f;
